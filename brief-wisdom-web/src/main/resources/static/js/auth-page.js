...
function switchAuthTab(tab) {
  ...
  if (!tabLogin || !loginForm || !phoneLoginForm || !registerForm) return;
...
async function doPhoneLogin() {
  ...
  if (!phone || !/^1[3-9]\d{9}$/.test(phone)) {
    showError("phoneLoginError", "请输入正确的手机号");
    return;
  }
  if (!code || code.length < 4) {
    showError("phoneLoginError", "请输入验证码");
    return;
  }
...
async function sendSmsCode(btnId, phoneInputId) {
  ...
  if (!phone || !/^1[3-9]\d{9}$/.test(phone)) {
    // 根据按钮所在位置显示对应错误提示
    const errorId =
      phoneInputId === "regPhone" ? "regError" : "phoneLoginError";
...
async function doRegister() {
  ...
  // 如果填写了手机号，必须填写验证码
  if (phone && !/^1[3-9]\d{9}$/.test(phone)) {
    showError("regError", "请输入正确的手机号");
    return;
  }
  if (phone && (!smsCode || smsCode.length < 4)) {
    showError("regError", "请填写短信验证码");
    return;
  }
