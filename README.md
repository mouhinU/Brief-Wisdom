...
- **AI 工具调用**：13 个 Function Calling 工具，覆盖知识检索、数据分析、集成自动化三大领域
...
> `init-20260629.sql` 为一体化初始化脚本，包含所有表结构、RBAC 权限、菜单树、AI 模型配置、简历示例数据。项目同时使用 Flyway
...
共 13 个工具，分三个层级，通过 `ToolConfig` 使用 `MethodToolCallbackProvider` 统一注册。
